 ...
 * 版本：2.5 播放service
 */
public class MusicPlayerService extends Service implements CacheListener {
...
//                        service.bumpSongCount(); //更新歌曲的播放次数
    ...
    private HttpProxyCacheServer getProxy() {
        return MusicApp.getProxy(getApplicationContext());
    }
    ...
    private void playCurrentAndNext() {
        synchronized (this) {
            ...
            if (mPlayingMusic.getUri() != null) {
                ...
                String proxyUrl = proxy.getProxyUrl(mPlayingMusic.getUri());
                onCacheListener = progress -> LogUtil.e(TAG, "-----progress" + progress);
                checkCachedState(proxyUrl);
//            if (checkCachedState(mPlayingMusic.getUri())) {
...
//                notifyChange(PLAY_STATE_CHANGED);
     ...
     * 检查缓存的状态
     */
    private boolean checkCachedState(String url) {
        HttpProxyCacheServer proxy = MusicApp.getProxy(this);
...
//        }
     ...
     * 获取正在播放进度
     */
    public long getCurrentCechePosition() {
        if (mPlayer != null && mPlayer.isInitialized()) {
            return mPlayer.position();
        } else {
            return 0;
