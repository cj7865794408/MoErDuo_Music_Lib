...
object MusicApiServiceImpl {
     ...
     *
     */
    fun getXJLyricInfo(music: Music): Observable<String> {
        val mLyricPath = FileUtils.getLrcDir() + "${music.title}-${music.artist}" + ".lrc"
        val vendor = music.type!!
        val mid = music.mid!!
        //网络歌词
        return if (FileUtils.exists(mLyricPath)) {
            ...
            var lrcList = music.lrcList
            var lrcStr = music.lyric
            if (lrcStr != null && !TextUtils.isEmpty(lrcStr)) {
                //保存文件
                val save = FileUtils.writeText(mLyricPath, lrcStr)
                LogUtil.e("保存网络歌词：$save")
                Observable.fromArray(lrcStr)
                result.onNext(lrcStr)
                result.onComplete()
            } else {
