        ...
        try {
            ...
            when (playMode) {
                PlayQueueManager.PLAY_MODE_LOOP -> {
                    imageView.setImageResource(R.drawable.ic_repeat_new)
...
//                    imageView.setImageResource(R.drawable.ic_shuffle_new)
...
//                }
    ...
    fun collectMusic(imageView: ImageView, music: Music?) {
        music?.let {
            imageView.setBackgroundResource(if (!it.isLove) R.mipmap.aixin_red_img else R.mipmap.aixin_gray_icon)
        }
            ...
            addListener(object : Animator.AnimatorListener {
                ...
                override fun onAnimationEnd(animation: Animator?) {
                    music?.let {
                        ...
                        EventBus.getDefault().post(InfoChangeEvent(Constants.PLAYLIST_LOVE_ID, music))
                    }
                }
...
fun Context.addDownloadQueue(result: Music) {
    ...
    val path = FileUtils.getMusicDir() + result.artistId + " - " + result.title + ".mp3"
