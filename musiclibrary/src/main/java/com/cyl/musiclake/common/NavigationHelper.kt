...
import com.cyl.musiclake.ui.main.MedMusicListAcitivty
...
//                    transitionView, MusicApp.getAppContext().getString(R.string.transition_cover))
    ...
    fun navigateToMusicPlaying(context: Activity, transitionView: View? = null, radiosData: SpecailRadioBean.DataBean.RadiosBean? = null) {
        val intent = Intent(context, MedMusicListAcitivty::class.java)
        Constants.radiosData = radiosData
        if (transitionView != null) {
            val compat = ActivityOptionsCompat.makeScaleUpAnimation(transitionView,
                    transitionView.width / 2, transitionView.height / 2, 0, 0)
//            val compat = ActivityOptionsCompat.makeSceneTransitionAnimation(context,
